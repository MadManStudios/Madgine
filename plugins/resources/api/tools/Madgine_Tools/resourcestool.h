...
namespace Tools {
    ...
    struct MADGINE_RESOURCES_TOOLS_EXPORT ResourcesTool : Tool<ResourcesTool> {
        ResourcesTool(ImRoot &root);
        ...
        void registerEditor(Resources::ResourceLoaderBase *loader, ResourceEditor *editor);

        void open(Resources::ResourceBase *res, Resources::ResourceLoaderBase *loader);
    ...
    private:
        ...
        struct ResourceEntry {
            Resources::ResourceBase *mResource;
            Resources::ResourceLoaderBase *mLoader;
        };

        ImVector<ResourceEntry> mResources;

        std::map<Resources::ResourceLoaderBase *, ResourceEditor *> mEditors;

        bool mDeleteRequested = false;
        float ZoomWheelAccum = 0.0f;
        float mIconSize = 32.0f;

        ResourceEditor *mLastFocusedEditor = nullptr;
    };
}
}