...
namespace Behavior {
    namespace Python3 {
        ...
        PyObject *PySender_await(PyObject *self)
        {
            ...
            construct(state->mInnerState, DelayedConstruct { [&]() { return Execution::connect(std::move(sender) | Execution::stoppable | Execution::with_debug_location(state->mChild), SenderReceiver { *state }); } });
