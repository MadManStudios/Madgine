...
namespace Behavior {
    ...
    struct BehaviorFactoryBase {
        ...
        virtual Behavior create(const UniqueOpaquePtr &handle, const Reflect::ArgumentList &args, std::vector<Behavior> behaviors) const = 0;
        virtual ParameterTuple createParameters(const UniqueOpaquePtr &handle) const = 0;
        virtual const BehaviorDescriptor &descriptor(const UniqueOpaquePtr &handle) const = 0;
    };
