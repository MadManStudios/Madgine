...
static void ImGui::ApplyLayoutClipRect(ImGuiLayout &layout)
{
    ...
    for (ImDrawCmd &cmd : layout.Splitter._Channels[1]._CmdBuffer) {
        ...
        if (cmd.ClipRect.x < current_clip_rect.x)
            cmd.ClipRect.x = current_clip_rect.x;
        if (cmd.ClipRect.y < current_clip_rect.y)
            cmd.ClipRect.y = current_clip_rect.y;
        if (cmd.ClipRect.z > current_clip_rect.z)
            cmd.ClipRect.z = current_clip_rect.z;
        if (cmd.ClipRect.w > current_clip_rect.w)
            cmd.ClipRect.w = current_clip_rect.w;
    }
