...
namespace Scene {
    ...
    using EntityContext = Behavior::ContextParameter<Entity::EntityPtr>;

    using SceneContext = Behavior::ContextParameter<SceneManager &>;

    inline constexpr auto wait_simulation = [](std::chrono::steady_clock::duration duration, SceneContext scene = {}) {
        return std::move(scene) | Execution::let_value([=](SceneManager &mgr) { return mgr.simulationClock().wait(duration); });
    };

    inline constexpr auto yield_simulation = [](SceneContext scene = {}) {
        return wait_simulation(0s, scene);
    };

    inline constexpr auto wait_animation = [](std::chrono::steady_clock::duration duration, SceneContext scene = {}) {
        return std::move(scene) | Execution::let_value([=](SceneManager &mgr) { return mgr.animationClock().wait(duration); });
    };

    inline constexpr auto yield_animation = [](SceneContext scene = {}) {
    ...
    inline constexpr auto look_at = [](Entity::EntityPtr target, bool locked = false, EntityContext entity = {}) {
