...
namespace Tools {
    ...
    static Math::Plane cameraPlane(const Render::Camera &camera, const Math::Vector3 &point, const Math::Vector3 *axis1 = nullptr, const Math::Vector3 *axis2 = nullptr)
    {

        const Math::Ray3 &ray = camera.toRay();
    ...
    bool SceneView::render()
    {
        ...
        if (mEditor.tool().beginSubPanel(("SceneView" + std::to_string(mIndex)).c_str(), &open, ImGuiDir_Up)) {
            ...
            Math::Vector2i iRegion { static_cast<int>(region.x), static_cast<int>(region.y) };
            ...
            if (ImGui::IsItemHovered() || ImGui::IsItemActive()) {
                Math::Vector2 pos = ImGui::GetItemRectMin();
                Math::Vector2 size = ImGui::GetItemRectSize();

                io3D.mNextFrameMouseRay = mCamera.mousePointToRay(Math::Vector2 { io.MousePos } - pos, size);
            }

            if (ImGui::IsItemActive() && mouseButton == 0 && mAxisDragging) {
                Math::Vector3 axis = axes[mDraggedAxis];

                Math::Plane targetPlane = cameraPlane(mCamera, mDragStoredPosition, &axis);

                if (auto intersection = Intersect(ray, targetPlane)) {

                    Math::Vector3 distance = ray.point(intersection[0]) - mDragStoredPosition - mDragRelMousePosition;
            ...
            if (ImGui::BeginDragDropTarget()) {
                Math::Vector3 pos = ray.point(5.0f);
                UndoStack stack;
