...
namespace Memory {
    ...
    namespace __generic_impl__ {
        ...
        struct BucketStrategyLinear {
            ...
            {
                return Math::clamp<size_t>((size - BucketSizeBegin - 1) / BucketSizeStep, 0, sizes::size - 1);
            }
        ...
        struct BucketStrategyLog {
            ...
            static size_t select(size_t size)
            {
                return Math::clamp<size_t>(static_cast<size_t>(std::ceil(logf(static_cast<float>(size) / BucketSizeBegin) / logf(BucketSizeFactor))), 0, sizes::size - 1);
            }
