#pragma once

namespace Engine {
namespace Serialize {

    struct SyncFunction {
        void (*mWriteFunctionArguments)(const std::vector<WriteMessage> &, const void *);
        void (*mWriteFunctionResult)(FormattedSerializeStream &, const void *);
        StreamResult (*mReadFunctionAction)(SyncableUnitBase *, FormattedSerializeStream &, uint16_t, FunctionType, PendingRequest &);
        StreamResult (*mReadFunctionRequest)(SyncableUnitBase *, FormattedMessageStream &, uint16_t, FunctionType, MessageId);
    };

}
}