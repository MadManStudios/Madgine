...
namespace Reflect {
    ...
    struct META_EXPORT ResultBuilder {
        ...
        ResultBuilder(Enum type, const char *function = nullptr, const char *file = nullptr, size_t line = 0)
            : mType(type)
            , mFunction(function)
            , mFile(file)
            , mLine(line)
...
#define REFLECT_ERROR_NOTRACE(Type) \
    ::Engine::Reflect::ResultBuilder { Type }

#define REFLECT_UNKNOWN_ERROR() REFLECT_ERROR(::Engine::GenericResult { ::Engine::GenericResult::UNKNOWN_ERROR })

#define REFLECT_UNKNOWN_ERROR_NOTRACE() REFLECT_ERROR_NOTRACE(::Engine::GenericResult { ::Engine::GenericResult::UNKNOWN_ERROR })
