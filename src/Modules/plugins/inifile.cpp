...
namespace Plugins {
    ...
    bool IniFile::saveToDisk(const Platform::Filesystem::Path &path) const
    {
        Stream stream = Platform::Filesystem::openFileWrite(path);
        if (!stream) {
    ...
    bool IniFile::loadFromDisk(const Platform::Filesystem::Path &path)
    {
        Stream stream = Platform::Filesystem::openFileRead(path);
        if (!stream)
            return false;
        mSections.clear();
        std::string line;
