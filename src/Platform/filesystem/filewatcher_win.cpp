...
#    if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_APP | WINAPI_PARTITION_SYSTEM)
        ...
        Containers::Generator<std::vector<FileEvent>> fileWatch(const Path &path)
        {
...
#    if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_APP | WINAPI_PARTITION_SYSTEM)
            assert(isDir(path));
            auto pib = mWatches.try_emplace(path);
            if (pib.second) {
                pib.first->second.setupAs<Containers::Generator<std::vector<FileEvent>>>() = fileWatch(pib.first->first);
            }
#    endif
        }
...
#    if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_APP | WINAPI_PARTITION_SYSTEM)
            ...
            if (it != mWatches.end()) {
                // cause destroy
                it->second.release<Containers::Generator<std::vector<FileEvent>>>();
                mWatches.erase(it);
            }
#    endif
        }
...
#    if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_APP | WINAPI_PARTITION_SYSTEM)
            for (std::pair<const Path, UniqueOpaquePtr> &handle : mWatches) {
                // cause destroy
                handle.second.release<Containers::Generator<std::vector<FileEvent>>>();
            }
            mWatches.clear();
#    endif
        }
...
#    if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_APP | WINAPI_PARTITION_SYSTEM)
            for (std::pair<const Path, UniqueOpaquePtr> &watch : mWatches) {
                Containers::Generator<std::vector<FileEvent>> &gen = watch.second.as<Containers::Generator<std::vector<FileEvent>>>();
                gen.next();
