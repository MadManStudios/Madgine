...
namespace Platform {
    namespace Fetch {
        ...
        template <>
        PLATFORM_EXPORT void FetchImpl<std::string>::receive(const void *buffer, size_t nmemb)
        {
        ...
        template <>
        PLATFORM_EXPORT void FetchImpl<JsonParser>::receive(const void *buffer, size_t nmemb)
        {
            mResult.parse({ static_cast<const char *>(buffer), nmemb });
        }

        template <>
        PLATFORM_EXPORT void FetchImpl<std::vector<std::byte>>::receive(const void *buffer, size_t nmemb)
        {
            size_t oldSize = mResult.size();
            mResult.resize(oldSize + nmemb);
