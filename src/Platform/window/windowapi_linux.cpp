...
namespace Platform {
    namespace Window {
        ...
        struct LinuxWindow final : OSWindow {
            LinuxWindow(::Window hwnd, const PlatformVector &size)
                : OSWindow(hwnd)
                , mSize(size)
            {
            }
            ...
            bool handle(const XEvent &e)
            {
                ...
                case MotionNotify: {
                    ...
                    PlatformVector mousePos { xme.x, xme.y };
        ...
        OSWindow *sCreateWindow(const WindowSettings &settings)
        {
            ...
            if (!handle) {
                ...
                PlatformVector pos = settings.mData.mPosition.x < 0 || settings.mData.mPosition.y < 0 ? PlatformVector { 0, 0 } : settings.mData.mPosition;
